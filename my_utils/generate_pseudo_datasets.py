...
class Generate_Pseudo_Dataset(Dataset):
    def __init__(self, lmdb_dir, mask_dir, dataset, split, mode, input_size,
                 word_length, coco_instance_gt=False):
        super(Generate_Pseudo_Dataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            data = dict(img=img, sent=sent, target=mask, img_name=img_name, img_id=img_id, seg_cat=seg_cat, seg_id=seg_id,
                        coco_target_box=BoxAnns, coco_target_mask=MaskAnns, coco_target_cat=cat_names)
            return data
...
class Generate_Pseudo_Dataset_lvis_cutler(Dataset):
...
if __name__ == '__main__':
    dataset = 'refcoco'
    img_dir = './datasets/images/train2014/'  # path to images
    mask_dir = f'./datasets/masks/{dataset}/'  # path to GT masks
    train_data = Generate_Pseudo_Dataset(lmdb_dir=f'./datasets/lmdb/{dataset}/train.lmdb',
                                         mask_dir=mask_dir,
                                         dataset=dataset,
                                         split='train',
                                         mode='train',
